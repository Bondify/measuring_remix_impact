...
wins = pd.read_csv(r"C:\Users\santi\Google Drive\Master\Python\Dash\Github-Heroku\Measure Remix impact\data\data.csv",
...
wins_scored['year'] = [i.year for i in wins_scored['Win Date']] #[i[-4:] for i in wins_scored['Win Date']]
...
df = df.append(df_aux)
df['hover_text'] = '<b>Account Name:</b> ' + df['Account Name'] + '<br>' + '<b>Date:</b> ' +df['Win Date'].apply(lambda x: x.date().strftime("%d %b %Y"))
       ...
       html.Div([
           html.Div([
            ...
            html.Div([
                ...
                ], style={"width": "25%", "float": "left", "font-family":"Raleway, sans-serif",
...
def show_fig(hover_data, start_date, end_date):
    df_filtered = df.loc[(df['Win Date']>start_date)& (df['Win Date']<end_date),:]
    traces = [
    go.Scatter(
        x = df_filtered.score_agency_to_community,
        y = df_filtered.score_size_of_impact,
        
        hovertext = df_filtered['Win: Win Name'], #df['hover_text'].unique(),
        text = df_filtered['hover_text'],
        hovertemplate = 
        #'<br><b>Account Name</b>: %{name}<br>'+
        #'<b>%{text}</b><br>' + 
        '%{text}<br>' + 
        '<br><b>Win Name</b>: %{hovertext}<br>',
        
        ids = df_filtered['Win: Win Name'],
        ...
        showlegend = False,
        mode = 'markers',
        customdata = df_filtered['Win Description'],
    )]
